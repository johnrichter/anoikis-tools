...
- `tag-prefix.sh <tag>` -- SC-VERSIONING: a release tag's path prefix must equal this
  module's path from the repo root. anoikis-tools carries exactly one Go module, at the
...
- `no-committed-binaries.sh [root]` -- SC-DISTRIBUTION: no built binary may be committed to
  this tree; every per-OS/arch artifact ships through `release.yml` (CD) instead. A CI
  runner cross-compiles every target for this repo (Go's cross-compilation needs no native
  arm64-macOS hardware), so no committed-binary path was ever needed here.

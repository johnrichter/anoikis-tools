...
# (SC-VERSIONING; anoikis-tools' one Go module lives at the repo root, so it
...
# per-CLI-prefixed tag directory and per-artifact .sha256 sidecar.
#
...
#                        e.g. "anoikis"), and (unless PF_BIN_ENV overrides
#                        it) the exported env var name: PF_CLI_NAME with
#                        '-' -> '_', uppercased, suffixed "_BIN".
